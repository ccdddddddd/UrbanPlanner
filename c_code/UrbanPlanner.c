...
static void Decider(short PlannerLevel, double BasicsInfo_currentLaneFrontDis,
                    ...
                    *AvoOncomingVehInfo, const TypeStopSignInfo StopSignInfo,
                    const TypeAvoFailVehInfo *AvoFailVehInfo, short
...
static double SpeedPlanTrafficLight(double speed, double d_veh2int, double s_b,
  ...
  TypeGlobVars *GlobVars, double Parameters_l_veh, double
  c_CalibrationVars_SpeedPlanTraf, double d_CalibrationVars_SpeedPlanTraf,
  double e_CalibrationVars_SpeedPlanTraf, double f_CalibrationVars_SpeedPlanTraf,
...
static void b_sort(double x_data[], const int x_size[2], int idx_data[], int
                   idx_size[2]);
...
static double c_ppval(const double pp_breaks[5], const double pp_coefs[12],
                      double x);
static void c_sort(double x[132], int idx[132]);
...
static double e_ppval(const double pp_breaks[4], const double pp_coefs[12],
                      double x);
static void eml_find(const boolean_T x[5], int i_data[], int i_size[2]);
...
static void g_minimum(const double x[8], double *ex, int *idx);
static double h_minimum(const double x[8]);
static double i_minimum(const double x[9]);
                    ...
                    Parameters, struct1_T *Decision)
{
  ...
  double ChassisInfo_speed_tmp_tmp;
  double CurrentLaneFrontDis;
  ...
  double v_e;
  double w_lane;
  int FailIndex_data[5];
  int iidx_data[5];
  int CountLaneFailDis_size[2];
  int iidx_size[2];
  int FailIndex_size_idx_1;
  int i;
  int loop_ub;
  ...
  /* ----------------------------------------------------- */
  for (i = 0; i < 5; i++) {
    b_AvoFailVehInfo[i] = (AvoFailVehInfo->failLaneindex[i] ==
      BasicsInfo_currentLaneIndex);
  }

  eml_find(b_AvoFailVehInfo, iidx_data, iidx_size);
  FailIndex_size_idx_1 = iidx_size[1];
  loop_ub = iidx_size[1];
  if (loop_ub - 1 >= 0) {
    memcpy(&FailIndex_data[0], &iidx_data[0], (unsigned int)loop_ub * sizeof(int));
  }

  if (iidx_size[1] != 0) {
    double CountLaneFailDis_data[5];
    double CountLaneFailLen_data[5];
    double CountLaneFailVel_data[5];
    if (iidx_size[1] > 1) {
      CountLaneFailDis_size[0] = 1;
      CountLaneFailDis_size[1] = iidx_size[1];
      for (i = 0; i < FailIndex_size_idx_1; i++) {
        CountLaneFailDis_data[i] = AvoFailVehInfo->
          failLaneFrontDis[FailIndex_data[i] - 1];
      }

      b_sort(CountLaneFailDis_data, CountLaneFailDis_size, iidx_data, iidx_size);
      loop_ub = iidx_size[1];
      for (i = 0; i < loop_ub; i++) {
        CountLaneFailVel_data[i] = AvoFailVehInfo->
          failLaneFrontVel[FailIndex_data[iidx_data[i] - 1] - 1];
      }

      loop_ub = iidx_size[1];
      for (i = 0; i < loop_ub; i++) {
        CountLaneFailLen_data[i] = AvoFailVehInfo->
          failLaneFrontLen[FailIndex_data[iidx_data[i] - 1] - 1];
      }
    } else {
      CountLaneFailDis_data[0] = AvoFailVehInfo->
        failLaneFrontDis[FailIndex_data[0] - 1];
      CountLaneFailVel_data[0] = AvoFailVehInfo->
        failLaneFrontVel[FailIndex_data[0] - 1];
      CountLaneFailLen_data[0] = AvoFailVehInfo->
  ...
  if ((BasicsInfo_currentLaneIndex != TargetLaneIndex) &&
    ...
    if (ChassisInfo_speed < 5.0) {
      ...
      fzero(ChassisInfo_speed, S_end, w_lane, &a_soll_StopSign,
            &CurrentLaneFrontLen, &S_max);
      ...
      ChassisInfo_speed_tmp_tmp = 2.0 *
        CalibrationVars->TrajPlanLaneChange.a_min;
      b_ChassisInfo_speed[1] = (0.0 - ChassisInfo_speed * ChassisInfo_speed) /
        ChassisInfo_speed_tmp_tmp;
      a = maximum(b_ChassisInfo_speed);
      b_ChassisInfo_speed[0] = w_lane;
      ChassisInfo_speed_tmp = ChassisInfo_speed * t_lc;
      b_ChassisInfo_speed[1] = ChassisInfo_speed_tmp + 0.5 *
        CalibrationVars->TrajPlanLaneChange.a_min * t_lc * t_lc;
      CurrentLaneFrontLen = maximum(b_ChassisInfo_speed);
      a_soll_StopSign = w_lane * w_lane;
      b_ChassisInfo_speed[0] = sqrt(a * a - a_soll_StopSign);
      b_ChassisInfo_speed[1] = sqrt(CurrentLaneFrontLen * CurrentLaneFrontLen -
        a_soll_StopSign);
      CurrentLaneFrontLen = maximum(b_ChassisInfo_speed);
      ...
      if ((-TargetLaneBehindDis > S_max) && (TargetLaneFrontDis <=
        ...
        b_S_b_end[1] = ((S_b_end + maximum(b_ChassisInfo_speed) *
          ...
          TargetLaneFrontVel * TargetLaneFrontVel) / ChassisInfo_speed_tmp_tmp)
          + Parameters.l_veh;
        b_S_b_end[2] = CurrentLaneFrontLen;
        ...
        b_S_b_end[0] = S_max;
        b_S_b_end[1] = S_max - (V_c_end * V_c_end - w_lane) /
          ChassisInfo_speed_tmp_tmp;
        ...
        b_S_b_end[1] = (S_c_end - V_end *
                        CalibrationVars->TrajPlanLaneChange.t_re) - (V_c_end *
          V_c_end - V_end * V_end) / ChassisInfo_speed_tmp_tmp;
        b_S_b_end[2] = sqrt(a * a - a_soll_StopSign);
        S_max = d_minimum(b_S_b_end);
      }
      ...
      if ((d_veh2int >= S_end +
           ...
           Parameters.l_veh) || (d_veh2goal < d_veh2int)) {
        prereq6 = true;
      } else {
        prereq6 = false;
      }
           ...
           ChassisInfo_speed * (t_lc + CalibrationVars->TrajPlanLaneChange.t_re)))
      {
        ...
        S_max = V_end * V_end;
        CurrentLaneFrontLen = 2.0 * CalibrationVars->TrajPlanLaneChange.a_min;
        b_S_b_end[1] = (w_lane + (S_max - TargetLaneFrontVel *
          TargetLaneFrontVel) / CurrentLaneFrontLen) + Parameters.l_veh;
        ...
        w_lane = S_c_end - CalibrationVars->TrajPlanLaneChange.t_re * V_end;
        b_S_b_end[0] = w_lane;
        b_S_b_end[1] = w_lane - (V_c_end * V_c_end - S_max) /
          CurrentLaneFrontLen;
        b_S_b_end[2] = ChassisInfo_speed * t_lc + 0.5 *
        ...
        b_S_b_end[1] = ((S_b_end + TargetLaneFrontVel *
                         CalibrationVars->TrajPlanLaneChange.t_re) + (w_lane -
          TargetLaneFrontVel * TargetLaneFrontVel) / S_max) + Parameters.l_veh;
        b_S_b_end[2] = ChassisInfo_speed * t_lc + 0.5 *
          CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc * t_lc;
        S_min = b_maximum(b_S_b_end);

        /*              S_max=min([(S_c_end-t_re*V_end-l_veh) S_c_end-V_end*t_re-(V_c_end.^2-V_end.^2)/(2*a_min)-l_veh S_0+V_0*t_lc+0.5*a_max_comfort*t_lc*t_lc]); */
        CurrentLaneFrontLen = S_c_end - CalibrationVars->TrajPlanLaneChange.t_re
          * V_end;
        b_S_b_end[0] = CurrentLaneFrontLen;
        b_S_b_end[1] = CurrentLaneFrontLen - (V_c_end * V_c_end - w_lane) /
          S_max;
        b_S_b_end[2] = ChassisInfo_speed * t_lc + 0.5 *
          CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
        S_max = d_minimum(b_S_b_end);
      ...
      if (S_max >= S_min) {
        S_max = 0.5 * CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc *
          t_lc;
        a_soll_StopSign = a_soll_StopSign * a_soll_StopSign /
          CalibrationVars->TrajPlanLaneChange.a_max_comfort;
        if ((S_end <= (t_lc * c_minimum(b_ChassisInfo_speed) + S_max) -
             a_soll_StopSign) && (S_end >= (t_lc * maximum(b_ChassisInfo_speed)
              - S_max) + a_soll_StopSign)) {
          prereq5 = true;
        } else {
          prereq5 = false;
        }
      ...
      if (((S_c_end - S_b_end) - Parameters.l_veh > TargetLaneFrontVel *
           ...
           * t_lc) && (V_end <= ChassisInfo_speed +
                       CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc)
          && prereq5 && prereq6 && (TargetLaneBehindDis <= -Parameters.l_veh) &&
          (TargetLaneFrontDis >= 0.0)) {
        b_ChassisInfo_speed[0] = 0.0;
        b_ChassisInfo_speed[1] = CurrentLaneFrontVel +
          CalibrationVars->TrajPlanLaneChange.index_accel *
  ...
  if ((CurrentTargetLaneIndex != TargetLaneIndex) &&
    ...
    if (ChassisInfo_speed < 5.0) {
      ...
      b_ChassisInfo_speed[1] = ChassisInfo_speed * t_lc + 0.5 *
        CalibrationVars->TrajPlanLaneChange.a_min * t_lc * t_lc;
      CurrentLaneFrontLen = maximum(b_ChassisInfo_speed);
      ChassisInfo_speed_tmp_tmp = w_lane * w_lane;
      b_ChassisInfo_speed[0] = sqrt(a * a - ChassisInfo_speed_tmp_tmp);
      b_ChassisInfo_speed[1] = sqrt(CurrentLaneFrontLen * CurrentLaneFrontLen -
        ChassisInfo_speed_tmp_tmp);
      CurrentLaneFrontLen = maximum(b_ChassisInfo_speed);
      b_ChassisInfo_speed[0] = 0.0;
      b_ChassisInfo_speed[1] = v_e +
        ...
        CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc;
      TargetLaneFrontVel = maximum(b_ChassisInfo_speed);
      V_c_end = RightLaneFrontDis + 0.5 * (TargetLaneFrontVel + v_e) * t_lc;
      ...
      b_ChassisInfo_speed[1] = v_d + S_max *
        CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc;
      TargetLaneBehindVel = maximum(b_ChassisInfo_speed);
      S_b_end = RightLaneBehindDis + 0.5 * (TargetLaneBehindVel + v_d) * t_lc;
      t_mid = 0.5 * t_lc;
      S_max = v_d * t_lc;
      if ((-RightLaneBehindDis > S_max) && (RightLaneFrontDis <=
        ...
        /*                  b车不存在 c车存在 */
        /*  V_end=min([0.5*(V_0+V_c_end) ((S_end.^2+w_lane.^2).^0.5)/t_lc*2-V_0]); */
        V_end = sqrt(S_end * S_end + ChassisInfo_speed_tmp_tmp) / t_lc * 2.0 -
          ChassisInfo_speed;
        b_ChassisInfo_speed[0] = TargetLaneBehindVel - V_end;
        b_ChassisInfo_speed[1] = 0.0;
        b_S_b_end[0] = (S_b_end + TargetLaneBehindVel *
                        CalibrationVars->TrajPlanLaneChange.t_re) +
          Parameters.l_veh;
        b_S_b_end[1] = ((S_b_end + maximum(b_ChassisInfo_speed) *
                         CalibrationVars->TrajPlanLaneChange.t_re) + (V_end *
          V_end - TargetLaneBehindVel * TargetLaneBehindVel) /
        ...
        b_S_b_end[0] = w_lane;
        b_S_b_end[1] = w_lane - (TargetLaneFrontVel * TargetLaneFrontVel - V_end
          * V_end) / ChassisInfo_speed_tmp;
        b_S_b_end[2] = sqrt(a * a - ChassisInfo_speed_tmp_tmp);
        S_max = d_minimum(b_S_b_end);
      ...
      /*  距离路口过近时不允许换道 */
      if (((V_c_end - S_b_end) - Parameters.l_veh > TargetLaneBehindVel *
           CalibrationVars->TrajPlanLaneChange.t_re + (TargetLaneFrontVel *
            TargetLaneFrontVel - TargetLaneBehindVel * TargetLaneBehindVel) /
           (2.0 * CalibrationVars->TrajPlanLaneChange.a_min)) && (V_end >
           ChassisInfo_speed + CalibrationVars->TrajPlanLaneChange.a_min * t_lc)
          && (V_end < ChassisInfo_speed +
              CalibrationVars->TrajPlanLaneChange.a_max * t_lc) && prereq5 &&
          prereq6 && (RightLaneBehindDis <= -Parameters.l_veh) &&
      ...
      if (-RightLaneBehindDis > S_max) {
        ...
        if (RightLaneFrontDis <= a_soll_StopSign) {
          /* , */
          /*                  b车不存在 c车存在 */
          b_ChassisInfo_speed[0] = 0.5 * (ChassisInfo_speed + TargetLaneFrontVel);
          b_ChassisInfo_speed[1] = ChassisInfo_speed;
          V_end = c_minimum(b_ChassisInfo_speed);
          w_lane = S_b_end + TargetLaneBehindVel *
            CalibrationVars->TrajPlanLaneChange.t_re;
          b_S_b_end[0] = w_lane + Parameters.l_veh;
          S_max = 2.0 * CalibrationVars->TrajPlanLaneChange.a_min;
          b_S_b_end[1] = (w_lane + (V_end * V_end - TargetLaneBehindVel *
            TargetLaneBehindVel) / S_max) + Parameters.l_veh;
          b_S_b_end[2] = a_soll_StopSign + 0.5 *
            CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc * t_lc;
          S_min = b_maximum(b_S_b_end);
          ...
          w_lane = V_c_end - CalibrationVars->TrajPlanLaneChange.t_re * V_end;
          b_S_b_end[0] = w_lane;
          b_S_b_end[1] = w_lane - (TargetLaneFrontVel * TargetLaneFrontVel -
            V_end * V_end) / S_max;
          b_S_b_end[2] = a_soll_StopSign + 0.5 *
            CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
          S_max = d_minimum(b_S_b_end);
      ...
      if (guard2) {
        if (-RightLaneBehindDis > S_max) {
          ...
          if (RightLaneFrontDis > S_end) {
            ...
            w_lane = ChassisInfo_speed * ChassisInfo_speed;
            S_max = 2.0 * CalibrationVars->TrajPlanLaneChange.a_min;
            b_S_b_end[1] = ((S_b_end + TargetLaneBehindVel *
                             CalibrationVars->TrajPlanLaneChange.t_re) + (w_lane
              - TargetLaneBehindVel * TargetLaneBehindVel) / S_max) +
              Parameters.l_veh;
            b_S_b_end[2] = ChassisInfo_speed * t_lc + 0.5 *
              CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc * t_lc;
            S_min = b_maximum(b_S_b_end);
            ...
            /*              S_max=min([(S_e_end-t_re*V_end-l_veh) S_e_end-V_end*t_re-(V_e_end.^2-V_end.^2)/(2*a_min)-l_veh S_0+V_0*t_lc+0.5*a_max_comfort*t_lc*t_lc]); */
            CurrentLaneFrontLen = V_c_end -
              CalibrationVars->TrajPlanLaneChange.t_re * ChassisInfo_speed;
            b_S_b_end[0] = CurrentLaneFrontLen;
            b_S_b_end[1] = CurrentLaneFrontLen - (TargetLaneFrontVel *
              TargetLaneFrontVel - w_lane) / S_max;
            b_S_b_end[2] = ChassisInfo_speed * t_lc + 0.5 *
              CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
            S_max = d_minimum(b_S_b_end);
          } else {
      ...
      if (guard1) {
        /*                  b车存在 c车存在 */
        b_ChassisInfo_speed[0] = ChassisInfo_speed;
        b_ChassisInfo_speed[1] = 0.5 * (TargetLaneBehindVel + TargetLaneFrontVel);
        V_end = c_minimum(b_ChassisInfo_speed);
        b_S_b_end[0] = (S_b_end + TargetLaneBehindVel *
                        CalibrationVars->TrajPlanLaneChange.t_re) +
          Parameters.l_veh;
      ...
      b_ChassisInfo_speed[0] = V_end;
      b_ChassisInfo_speed[1] = ChassisInfo_speed;
      CurrentLaneFrontLen = CalibrationVars->TrajPlanLaneChange.a_max_comfort *
        t_lc;
      a_soll_StopSign = (CurrentLaneFrontLen - fabs(V_end - ChassisInfo_speed)) /
        2.0;
      ...
      if (((V_c_end - S_b_end) - Parameters.l_veh > TargetLaneBehindVel *
           ...
           (2.0 * CalibrationVars->TrajPlanLaneChange.a_min)) && (V_end >=
           ChassisInfo_speed + CalibrationVars->TrajPlanLaneChange.a_min_comfort
           * t_lc) && (V_end <= ChassisInfo_speed + CurrentLaneFrontLen) &&
          prereq5 && prereq6 && (RightLaneBehindDis <= -Parameters.l_veh) &&
          (RightLaneFrontDis >= 0.0)) {
        b_ChassisInfo_speed[0] = 0.0;
        b_ChassisInfo_speed[1] = CurrentLaneFrontVel + ChassisInfo_speed_tmp *
          t_mid;
        if ((CurrentLaneFrontDis + 0.5 * (maximum(b_ChassisInfo_speed) +
              CurrentLaneFrontVel) * t_mid > 0.5 * S_end) &&
            (CurrentLaneFrontDis >= 0.25 * t_lc * (0.75 * ChassisInfo_speed +
  ...
  if ((CurrentLaneFrontDis < 195.0) && (CurrentLaneFrontVel < 20.0)) {
  ...
  if ((GlosaActive == 1) && (TrafficLightActive == 1) && (d_veh2Intstopline >
    ...
    scen_glosa(d_veh2Intstopline, ChassisInfo_speed, TrafficLightInfo->phase,
               ...
               CalibrationVars->Decider.dMin, &CurrentLaneFrontLen, &S_max,
               &a_soll_StopSign);
  ...
  /*  end */
  if ((CurrentLaneFrontVel <= 0.5) && (CurrentLaneFrontDis <= dist_wait +
       CalibrationVars->ACC.d_wait)) {
  ...
  if ((GlosaActive == 1) && (TrafficLightActive == 1) && (d_veh2Intstopline >
       0.0)) {
    if ((S_max == -1.0) && (d_veh2Intstopline <= dist_wait)) {
  ...
  S_b_end = h_minimum(wait_matrix);
  if (S_b_end <= dist_wait) {
    FailIndex_size_idx_1 = 0;
    exitg1 = false;
    while ((!exitg1) && (FailIndex_size_idx_1 < 8)) {
      if (wait_matrix[FailIndex_size_idx_1] == S_b_end) {
        Wait = (short)(FailIndex_size_idx_1 + 1);
        exitg1 = true;
      } else {
        FailIndex_size_idx_1++;
      }
    }
  } else {
    S_b_end = 200.0;
  ...
  if (BasicsInfo_d_veh2goal < 60.0) {
    ...
    if (BasicsInfo_currentLaneIndex != TargetLaneIndex) {
      ...
      if (BasicsInfo_d_veh2goal < (CalibrationVars->TrajPlanLaneChange.v_max_int
        ...
        S_max = ACC(8.3333333333333339, 20.0, 200.0, ChassisInfo_speed, 0.0,
                    CalibrationVars->ACC.a_max, CalibrationVars->ACC.a_min,
                    CalibrationVars->ACC.d_wait2faultyCar,
                    CalibrationVars->ACC.tau_v_com, CalibrationVars->ACC.tau_v,
                    CalibrationVars->ACC.tau_d, CalibrationVars->ACC.tau_v_bre,
      ...
      S_max = ACC(BasicsInfo_v_max, 0.0, BasicsInfo_d_veh2goal +
  ...
  a_soll_matrix[7] = S_max;
  a_soll_matrix[8] = CurrentLaneFrontLen;
  a_soll_StopSign = i_minimum(a_soll_matrix);
  ...
  if (GlobVars->Decider.dec_follow == 0) {
    /* 跟车减速指示的决策 */
    if ((CurrentLaneFrontVel * CurrentLaneFrontVel - ChassisInfo_speed *
         ChassisInfo_speed) * 0.5 / CalibrationVars->Decider.a_bre +
        CalibrationVars->ACC.d_wait > CurrentLaneFrontDis) {
      dec_follow = 1;
    }
  } else if ((CurrentLaneFrontVel * CurrentLaneFrontVel - ChassisInfo_speed *
              ChassisInfo_speed) * 0.5 / CalibrationVars->Decider.a_bre_com +
             CalibrationVars->ACC.d_wait < CurrentLaneFrontDis) {
    dec_follow = 0;
  }
  ...
  if (PlannerLevel == 2) {
    ...
    a_soll_index = 9;
    exitg1 = false;
    while ((!exitg1) && (a_soll_index > 0)) {
      dir_start = a_soll_index;

      /* 从后往前查 */
      if (a_soll_matrix[a_soll_index - 1] == a_soll_StopSign) {
        exitg1 = true;
      } else {
        a_soll_index--;
      }
    ...
    CurrentLaneFrontLen = (TargetVelocity * TargetVelocity - 69.444444444444457)
      * 0.5 / CalibrationVars->Decider.a_bre_com;
    ...
    if ((a_soll_StopSign < -0.2) && (Wait == 0)) {
      ...
      if (SlowDown == 7) {
        ...
        {
          if (CurrentLaneFrontDis <= CurrentLaneFrontLen + dist_wait) {
            TargetVelocity *= 3.6;
            TargetVelocity = rt_roundd_snf(TargetVelocity);

            /* km/h */
          } else if (CurrentLaneFrontDis <= (CurrentLaneFrontLen + S_max) +
                     dist_wait) {
            TargetVelocity = 30.0;
        ...
        if (StopSignInfo.d_veh2stopline <= CurrentLaneFrontLen + dist_wait) {
          TargetVelocity *= 3.6;
          TargetVelocity = rt_roundd_snf(TargetVelocity);

          /* km/h */
        } else if (StopSignInfo.d_veh2stopline <= (CurrentLaneFrontLen + S_max)
                   + dist_wait) {
          TargetVelocity = 30.0;
        ...
        if (d_veh2Rampstopline <= CurrentLaneFrontLen + dist_wait) {
          TargetVelocity *= 3.6;
          TargetVelocity = rt_roundd_snf(TargetVelocity);

          /* km/h */
        } else if (d_veh2Rampstopline <= (CurrentLaneFrontLen + S_max) +
                   dist_wait) {
          TargetVelocity = 30.0;
        ...
        /* oncommingcar */
        if (d_veh2waitingArea <= CurrentLaneFrontLen + dist_wait) {
          TargetVelocity *= 3.6;
          TargetVelocity = rt_roundd_snf(TargetVelocity);

          /* km/h */
        } else if (d_veh2waitingArea <= (CurrentLaneFrontLen + S_max) +
                   dist_wait) {
          TargetVelocity = 30.0;
        ...
        if (GlobVars->SpeedPlanAvoidPedestrian.wait_ped == 1) {
          if (d_veh2stopline_ped <= CurrentLaneFrontLen + dist_wait) {
            TargetVelocity *= 3.6;
            TargetVelocity = rt_roundd_snf(TargetVelocity);

            /* km/h */
          } else if (d_veh2stopline_ped <= (CurrentLaneFrontLen + S_max) +
                     dist_wait) {
            TargetVelocity = 30.0;
        ...
        } else if (d_veh2cross <= (CurrentLaneFrontLen + S_max) + dist_wait) {
          /* 人行道限速 */
          TargetVelocity = 30.0;

          /* km/h */
        } else if (d_veh2cross <= ((CurrentLaneFrontLen + S_max) +
                    TargetLaneBehindVel) + dist_wait) {
          TargetVelocity = 40.0;
        ...
        if (GlobVars->SpeedPlanTrafficLight.wait_TrafficLight == 1) {
          if (d_veh2Intstopline <= CurrentLaneFrontLen + dist_wait) {
            TargetVelocity *= 3.6;
            TargetVelocity = rt_roundd_snf(TargetVelocity);

            /* km/h */
          } else if ((d_veh2Intstopline <= (CurrentLaneFrontLen + S_max) +
          ...
          if (d_veh2Intstopline <= (CurrentLaneFrontLen + S_max) + dist_wait) {
            TargetVelocity = 30.0;
        ...
        /* 靠边停车 */
        if (d_veh2goal <= CurrentLaneFrontLen + dist_wait) {
          TargetVelocity *= 3.6;
          TargetVelocity = rt_roundd_snf(TargetVelocity);

          /* km/h */
        } else if (d_veh2goal <= (CurrentLaneFrontLen + S_max) + dist_wait) {
          TargetVelocity = 30.0;

          /* km/h */
        } else if (d_veh2goal <= ((CurrentLaneFrontLen + S_max) +
                    TargetLaneBehindVel) + dist_wait) {
          TargetVelocity = 40.0;
    ...
    } else if ((CurrentLaneFrontDis < (CurrentLaneFrontVel * CurrentLaneFrontVel
      - ChassisInfo_speed * ChassisInfo_speed) * 0.5 /
                CalibrationVars->Decider.a_bre) && (Wait == 0)) {
      /* wait时速度默认值 */
      SlowDown = 7;
      if ((CurrentLaneFrontVel <= 0.1) && (CurrentLaneFrontDis >= dist_wait)) {
        if (CurrentLaneFrontDis <= CurrentLaneFrontLen + dist_wait) {
          ...
          S_max += CurrentLaneFrontLen;
          if (CurrentLaneFrontDis <= S_max + dist_wait) {
            TargetVelocity = 30.0;

            /* km/h */
          } else if (CurrentLaneFrontDis <= (S_max + TargetLaneBehindVel) +
                     dist_wait) {
            TargetVelocity = 40.0;
  ...
  Parameters_l_veh)
{
  ...
  if (dec_avoidOncomingVehicle == 1) {
    ...
    while ((!exitg1) && (i < 6)) {
      ...
      if ((s_veh[i] <= d_veh[i]) || (s_vehapostrophe[i] > -4.0)) {
        /* 20230714 */
  ...
  if (wait_avoidOncomingVehicle == 1) {
    ...
    while ((!exitg1) && (i < 6)) {
      ...
      if ((d_veh2waitingArea < 10.0) && (d_veh[i] > d_veh2conflict[i] +
           Parameters_l_veh) && (s_vehapostrophe[i] < -4.0)) {
        i++;
      } else {
        /* 20230714 */
        wait_avoidOncomingVehicle = 1;
  ...
  const CalibACC *CalibrationVars_ACC)
{
  ...
  if (GlobVars->SpeedPlanTrafficLight.dec_bre_TrafficLight == 0) {
    if ((d_veh2int <= (0.0 - speed * speed) / (2.0 *
          e_CalibrationVars_SpeedPlanTraf) + 10.0) && (d_veh2int > 0.0)) {
      /*  原为10 12.31修改1,2023/9/5修改为0  */
      dec_bre = 1;
    }
  ...
  if ((greenLight != 1.0) && (d_veh2int <= 0.0) && (-Parameters_l_veh <= 0.0)) {
    b_wait = 1;
  }
  ...
  traj_omega[80])
{
  ...
  if ((d_veh2goal < 40.0) && (GoalLaneIndex != CurrentLaneIndex)) {
    S_min_dyn = WidthOfLanes[GoalLaneIndex - 1];
    WidthOfLanes[GoalLaneIndex - 1] = S_min_dyn + 2.0 * ((0.5 * S_min_dyn - 0.5 *
  ...
  S_min_dyn = 0.5 * WidthOfLanes[CurrentLaneIndex - 1];
  if ((short)i < 1) {
    i = 1;
  } else {
    i = (short)i;
  ...
  w_lane_left = 0.5 * WidthOfLanes[i - 1] + S_min_dyn;
  ...
  w_lane_right = 0.5 * WidthOfLanes[i - 1] + S_min_dyn;
  ...
  *a_soll = 100.0;
  ...
  if ((GlobVars->TrajPlanLaneChange.countLaneChange == 0) && (CurrentLaneIndex
    ...
    if (speed < 5.0) {
      ...
      fzero(speed, S_end, w_lane, &S_d_end, &S_min_dyn, &S_b_end);
      b_speed[0] = t_lc;
      b_speed[1] = S_d_end;
      t_lc = 0.1 * rt_roundd_snf(maximum(b_speed) / 0.1);
      b_speed[0] = w_lane;
      S_d_end = 2.0 * CalibrationVars->TrajPlanLaneChange.a_min;
      b_speed[1] = (0.0 - speed * speed) / S_d_end;
      a = maximum(b_speed);
      b_speed[0] = w_lane;
      ...
      if ((-TargetLaneBehindDis > S_b_end_tmp) && (TargetLaneFrontDis <= speed *
        ...
        /*                  b车不存在 c车存在 */
        /*  V_end=min([0.5*(V_0+V_c_end) ((S_end.^2+w_lane.^2).^0.5)/t_lc*2-V_0]); */
        TargetLaneFrontVel = w_lane * w_lane;
        V_end = sqrt(S_end * S_end + TargetLaneFrontVel) / t_lc * 2.0 - speed;
        ...
        b_S_b_end[1] = ((S_b_end + maximum(b_speed) *
                         CalibrationVars->TrajPlanLaneChange.t_re) +
                        (S_b_end_tmp - V_b_end * V_b_end) / S_d_end) +
          Parameters.l_veh;
        ...
        b_S_b_end[1] = b_S_b_end_tmp - (V_c_end * V_c_end - S_b_end_tmp) /
          S_d_end;
        b_S_b_end[2] = sqrt(a * a - TargetLaneFrontVel);
        ...
        b_S_b_end[1] = b_S_b_end_tmp - (V_c_end * V_c_end - S_b_end_tmp) /
          S_d_end;
        b_S_b_end[2] = sqrt(a * a - w_lane * w_lane);
        TargetLaneFrontVel = d_minimum(b_S_b_end);
      } else if ((-TargetLaneBehindDis > S_b_end_tmp) && (TargetLaneFrontDis >
                  speed * t_lc)) {
        /* , */
      ...
      if ((-TargetLaneBehindDis > S_b_end_tmp) && (TargetLaneFrontDis <= speed *
        ...
        b_S_b_end_tmp = V_end * V_end;
        S_min_dyn = 2.0 * CalibrationVars->TrajPlanLaneChange.a_min;
        b_S_b_end[1] = (S_b_end_tmp + (b_S_b_end_tmp - V_b_end * V_b_end) /
        ...
        b_S_b_end[1] = S_b_end_tmp - (V_c_end * V_c_end - b_S_b_end_tmp) /
          S_min_dyn;
        b_S_b_end[2] = speed * t_lc + 0.5 *
          CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
        TargetLaneFrontVel = d_minimum(b_S_b_end);
        ...
        b_S_b_end_tmp = S_c_end - CalibrationVars->TrajPlanLaneChange.t_re *
          V_end;
        b_S_b_end[0] = b_S_b_end_tmp;
        b_S_b_end[1] = b_S_b_end_tmp - (V_c_end * V_c_end - S_b_end_tmp) / (2.0 *
          CalibrationVars->TrajPlanLaneChange.a_min);
        b_S_b_end[2] = speed * t_lc + 0.5 *
          CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
        TargetLaneFrontVel = d_minimum(b_S_b_end);
        ...
        b_S_b_end[1] = ((S_b_end + V_b_end *
                         CalibrationVars->TrajPlanLaneChange.t_re) +
                        (S_b_end_tmp - V_b_end * V_b_end) / b_S_b_end_tmp) +
          Parameters.l_veh;
        b_S_b_end[2] = speed * t_lc + 0.5 *
          CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc * t_lc;
        S_min = b_maximum(b_S_b_end);
        ...
        /*              S_max=min([(S_c_end-t_re*V_end-l_veh) S_c_end-V_end*t_re-(V_c_end.^2-V_end.^2)/(2*a_min)-l_veh S_0+V_0*t_lc+0.5*a_max_comfort*t_lc*t_lc]); */
        S_min_dyn = S_c_end - CalibrationVars->TrajPlanLaneChange.t_re * speed;
      ...
      b_speed[0] = V_end;
      b_speed[1] = speed;
      b_S_b_end_tmp = (CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc
      ...
      if (TargetLaneFrontVel >= S_min) {
        S_b_end_tmp = 0.5 * CalibrationVars->TrajPlanLaneChange.a_max_comfort *
          t_lc * t_lc;
        TargetLaneFrontVel = b_S_b_end_tmp * b_S_b_end_tmp /
          CalibrationVars->TrajPlanLaneChange.a_max_comfort;
        if ((S_end <= (t_lc * c_minimum(b_speed) + S_b_end_tmp) -
             TargetLaneFrontVel) && (S_end >= (t_lc * maximum(b_speed) -
  ...
  if ((CountLaneChange == 0) && (CurrentLaneIndex != TargetLaneIndex) &&
    ...
    if (speed < 5.0) {
      ...
      w_lane_right = s_e + 0.5 * (TargetLaneBehindVel + v_e) * t_lc;
      ...
      if ((-s_d > S_b_end_tmp) && (s_e <= speed * t_lc)) {
        ...
        a = speed * t_lc + 0.5 * CalibrationVars->TrajPlanLaneChange.a_max *
          t_lc * t_lc;
        b_S_b_end_tmp = w_lane_right - CalibrationVars->TrajPlanLaneChange.t_re *
          V_end;
        b_S_b_end[0] = b_S_b_end_tmp;
        b_S_b_end[1] = b_S_b_end_tmp - (TargetLaneBehindVel *
        ...
        a = speed * t_lc + 0.5 * CalibrationVars->TrajPlanLaneChange.a_max *
          t_lc * t_lc;
        b_S_b_end[0] = w_lane_right - CalibrationVars->TrajPlanLaneChange.t_re *
          V_end;
        b_S_b_end[1] = (w_lane_right - V_end *
                        CalibrationVars->TrajPlanLaneChange.t_re) -
        ...
        a = speed * t_lc + 0.5 * CalibrationVars->TrajPlanLaneChange.a_max *
          t_lc * t_lc;
        S_b_end_tmp = w_lane_right - CalibrationVars->TrajPlanLaneChange.t_re *
          V_end;
        b_S_b_end[0] = S_b_end_tmp;
      ...
      guard2 = false;
      if (-s_d > S_b_end_tmp) {
        TargetLaneFrontVel = speed * t_lc;
        if (s_e <= TargetLaneFrontVel) {
          /* , */
          /*                  b车不存在 c车存在 */
          ...
          b_S_b_end[2] = TargetLaneFrontVel + 0.5 *
            CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc * t_lc;
          S_min = b_maximum(b_S_b_end);
          ...
          b_S_b_end[1] = S_b_end_tmp - (TargetLaneBehindVel *
            TargetLaneBehindVel - V_end * V_end) / b_S_b_end_tmp;
          b_S_b_end[2] = TargetLaneFrontVel + 0.5 *
            CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
          TargetLaneFrontVel = d_minimum(b_S_b_end);
          b_S_b_end[0] = S_min;
      ...
      if (guard2) {
        if ((-s_d <= S_b_end_tmp) && (s_e > speed * t_lc)) {
          ...
          b_S_b_end[0] = S_b_end_tmp + Parameters.l_veh;
          b_S_b_end_tmp = V_end * V_end;
          b_S_b_end[1] = (S_b_end_tmp + (b_S_b_end_tmp - S_b_end * S_b_end) /
          ...
          b_S_b_end[0] = S_b_end_tmp;
          b_S_b_end[1] = S_b_end_tmp - (TargetLaneBehindVel *
            TargetLaneBehindVel - b_S_b_end_tmp) / (2.0 *
            CalibrationVars->TrajPlanLaneChange.a_min);
          b_S_b_end[2] = speed * t_lc + 0.5 *
            CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
          TargetLaneFrontVel = d_minimum(b_S_b_end);
          ...
          if (s_e > S_end) {
            ...
            b_S_b_end[1] = ((S_d_end + S_b_end *
                             CalibrationVars->TrajPlanLaneChange.t_re) +
                            (S_b_end_tmp - S_b_end * S_b_end) / b_S_b_end_tmp) +
              Parameters.l_veh;
            b_S_b_end[2] = speed * t_lc + 0.5 *
              CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc * t_lc;
            S_min = b_maximum(b_S_b_end);
            ...
            /*              S_max=min([(S_e_end-t_re*V_end-l_veh) S_e_end-V_end*t_re-(V_e_end.^2-V_end.^2)/(2*a_min)-l_veh S_0+V_0*t_lc+0.5*a_max_comfort*t_lc*t_lc]); */
            S_min_dyn = w_lane_right - CalibrationVars->TrajPlanLaneChange.t_re *
              speed;
            b_S_b_end[0] = S_min_dyn;
            b_S_b_end[1] = S_min_dyn - (TargetLaneBehindVel *
              TargetLaneBehindVel - S_b_end_tmp) / b_S_b_end_tmp;
            b_S_b_end[2] = speed * t_lc + 0.5 *
              CalibrationVars->TrajPlanLaneChange.a_max_comfort * t_lc * t_lc;
            TargetLaneFrontVel = d_minimum(b_S_b_end);
          } else {
      ...
      if (((w_lane_right - S_d_end) - Parameters.l_veh > S_b_end *
           CalibrationVars->TrajPlanLaneChange.t_re + (TargetLaneBehindVel *
            ...
            CalibrationVars->TrajPlanLaneChange.a_min)) && (V_end >= speed +
           CalibrationVars->TrajPlanLaneChange.a_min_comfort * t_lc) && (V_end <=
           speed + S_min_dyn) && prereq5 && prereq6 && (s_d <= -Parameters.l_veh)
          && (s_e >= 0.0)) {
        b_speed[0] = 0.0;
  ...
  if (CountLaneChange == 1) {
    ...
    a = 3.0 * para[0];
    S_b_end = 4.0 * para[1];
    S_min_dyn = 5.0 * para[2];
    for (SwitchACC = 0; SwitchACC < 100; SwitchACC++) {
      S_b_end_tmp = x1[SwitchACC];
      S_b_end_tmp = ((a * (S_b_end_tmp * S_b_end_tmp) + S_b_end * rt_powd_snf
                      (S_b_end_tmp, 3.0)) + S_min_dyn * rt_powd_snf(S_b_end_tmp,
        4.0)) / 1.0E+6;
      b[SwitchACC] = sqrt(S_b_end_tmp * S_b_end_tmp + 1.0);
    }

    double dv1[16];
    S_min_dyn = trapz(x1, b);
    para_ST[0] = 0.0;
    para_ST[1] = speed;
    para_ST[2] = S_min_dyn;
    para_ST[3] = V_end;
    ...
    for (SwitchACC = 0; SwitchACC < i; SwitchACC++) {
      S_min_dyn = 0.05 * (((double)SwitchACC + 1.0) - 1.0);
      S_b_end = S_min_dyn * S_min_dyn;
      S_traj[SwitchACC] = ((para_ST[0] + para_ST[1] * S_min_dyn) + para_ST[2] *
                           S_b_end) + para_ST[3] * rt_powd_snf(S_min_dyn, 3.0);
      V_traj[SwitchACC] = (para_ST[1] + para_ST[2] * 2.0 * S_min_dyn) + para_ST
        [3] * 3.0 * S_b_end;
    ...
    i = (int)(t_lc / 0.05);
    for (SwitchACC = 0; SwitchACC < i; SwitchACC++) {
      TargetLaneFrontVel = X_traj[SwitchACC + 1];
      GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC] = pos_s +
        TargetLaneFrontVel;
      i3 = CurrentLaneIndex - TargetLaneIndex;
      S_min_dyn = rt_powd_snf(TargetLaneFrontVel, 3.0);
      S_b_end = rt_powd_snf(TargetLaneFrontVel, 4.0);
      GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC + 120] = pos_l +
        (double)i3 * ((para[0] * S_min_dyn + para[1] * S_b_end) + para[2] *
                      rt_powd_snf(TargetLaneFrontVel, 5.0)) * 1.0E-6;
      TargetLaneFrontVel = atan(((para[0] * 3.0 * (TargetLaneFrontVel *
        TargetLaneFrontVel) + para[1] * 4.0 * S_min_dyn) + para[2] * 5.0 *
        S_b_end) * 1.0E-6);
      S_min_dyn = 90.0 - (double)i3 * 180.0 / 3.1415926535897931 *
        TargetLaneFrontVel;
      GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC + 240] = S_min_dyn;
      TargetLaneFrontVel *= 57.295779513082323;
      S_b_end = TargetLaneFrontVel;
      b_cosd(&S_b_end);
      S_d_end = V_traj[SwitchACC + 1];
      GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC + 360] = S_d_end *
        S_b_end;
      b_sind(&TargetLaneFrontVel);
      GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC + 480] = (double)i3 *
        S_d_end * TargetLaneFrontVel;
      if ((unsigned int)SwitchACC + 1U == 1U) {
        ...
        GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC + 600] =
          (S_min_dyn - GlobVars->TrajPlanLaneChange.laneChangePath[SwitchACC +
           239]) / 0.05;
      }
    }
  ...
  if (DurationLaneChange != 0) {
    ...
    b_speed[1] = TargetLaneFrontVel + 0.0 *
      CalibrationVars->TrajPlanLaneChange.a_min_comfort * S_min_dyn;
    V_c_end = maximum(b_speed);
    S_c_end = (TargetLaneFrontDis + pos_s) + 0.5 * (V_c_end + TargetLaneFrontVel)
      * S_min_dyn;
    b_speed[0] = 0.0;
    b_speed[1] = TargetLaneBehindVel + 0.0 *
      CalibrationVars->TrajPlanLaneChange.a_max_comfort * S_min_dyn;
    V_b_end = maximum(b_speed);
    ...
    if ((CurrentLaneIndex != CurrentTargetLaneIndex) && (DurationLaneChange != 0))
    {
      speed_tmp = ((TargetLaneBehindDis + pos_s) + 0.5 * (V_b_end +
        TargetLaneBehindVel) * S_min_dyn) + V_b_end *
        CalibrationVars->TrajPlanLaneChange.t_re;
      b_speed[0] = speed_tmp + Parameters.l_veh;
      S_min_dyn = GlobVars->TrajPlanLaneChange.laneChangePath[(int)S_d_end + 359];
      S_b_end = S_min_dyn * S_min_dyn;
      S_d_end = 2.0 * CalibrationVars->TrajPlanLaneChange.a_min;
      b_speed[1] = (speed_tmp + (S_b_end - V_b_end * V_b_end) / S_d_end) +
        Parameters.l_veh;
      guard1 = false;
      if (rt_roundd_snf(S_end * 1000.0) < rt_roundd_snf(maximum(b_speed) *
        ...
        b_speed[0] = S_c_end - CalibrationVars->TrajPlanLaneChange.t_re *
          S_min_dyn;
        b_speed[1] = S_c_end - (V_c_end * V_c_end - S_b_end) / S_d_end;
  ...
  if ((DurationLaneChange > 0) && (DurationLaneChange <= rt_roundd_snf(10.0 *
    ...
    TargetLaneFrontVel = (S_b_end_tmp + 1.0) - (double)DurationLaneChange;
    if (TargetLaneFrontVel < 32768.0) {
      if (TargetLaneFrontVel >= -32768.0) {
        i1 = (short)TargetLaneFrontVel;
      } else {
        i1 = MIN_int16_T;
      }
    } else if (TargetLaneFrontVel >= 32768.0) {
      i1 = MAX_int16_T;
    } else {
      i1 = 0;
    }
    ...
    if (i2 <= 120) {
      ...
      TargetLaneFrontVel = (rt_roundd_snf(t_lc_traj / 0.1) + 1.0) - (double)
        DurationLaneChange;
      if (TargetLaneFrontVel < 32768.0) {
        if (TargetLaneFrontVel >= -32768.0) {
          count_2 = (short)TargetLaneFrontVel;
        } else {
          count_2 = MIN_int16_T;
        }
      } else if (TargetLaneFrontVel >= 32768.0) {
                   ...
                   idx_size[2])
{
  ...
  ib = x_size[1];
  if (ib - 1 >= 0) {
    memset(&iwork_data[0], 0, (unsigned int)ib * sizeof(signed char));
  }
  ...
  if (i3 > 4) {
    for (i1 = 0; i1 < 5; i1++) {
      iwork_data[i1] = (signed char)idx_data[i1];
    ...
    do {
      exitg1 = 0;
      ib++;
      if (xwork_data[quartetOffset] <= xwork_data[4]) {
        idx_data[ib] = iwork_data[quartetOffset];
        x_data[ib] = xwork_data[quartetOffset];
        if (quartetOffset + 1 < 4) {
          quartetOffset++;
        } else {
          exitg1 = 1;
        }
      } else {
        idx_data[ib] = iwork_data[4];
        x_data[ib] = xwork_data[4];
        ib -= quartetOffset;
        for (i1 = quartetOffset + 1; i1 < 5; i1++) {
          i4 = ib + i1;
          idx_data[i4] = iwork_data[i1 - 1];
          x_data[i4] = xwork_data[i1 - 1];
        }
                  ...
                  struct2_T *Refline)
{
  ...
  double c_a_soll_SpeedPlanAvoidPedestri[8];
  ...
  minimum(AvoMainRoVehInfo->targetLaneFrontDisAvoidVehicle, &d_veh2waitingArea,
          &b_wait);
  ...
  if (trueCount != 0) {
    ...
    b_minimum(AvoMainRoVehInfo_data, AvoMainRoVehInfo_size, &d_veh2waitingArea,
              &b_wait);
    TargetLaneFrontDisAvoidVehicle =
  ...
  if (trueCount != 0) {
    if (trueCount == b_wait) {
      ...
      binary_expand_op(AvoMainRoVehInfo, d_tmp_data, tmp_size, e_tmp_data,
                       b_tmp_size, &d_veh2waitingArea, &b_wait);
    }
  ...
  if (vectorAny(AvoFailVehInfo->failLaneindex, iv1)) {
    ...
    for (trueCount = 0; trueCount < 5; trueCount++) {
      short i;
      i = AvoFailVehInfo->failLaneindex[trueCount];
      if ((i != 0) && (i == CurrentLaneIndex)) {
        a_soll_Fail = fmin(a_soll_Fail, ACC(BasicsInfo->v_max,
          AvoFailVehInfo->failLaneFrontVel[trueCount],
          FailLaneFrontDis[trueCount] - AvoFailVehInfo->
          failLaneFrontLen[trueCount], speed_tmp_tmp, 0.0,
          CalibrationVars->ACC.a_max, CalibrationVars->ACC.a_min,
          CalibrationVars->ACC.d_wait2faultyCar, CalibrationVars->ACC.tau_v_com,
          CalibrationVars->ACC.tau_v, CalibrationVars->ACC.tau_d,
          CalibrationVars->ACC.tau_v_bre, CalibrationVars->ACC.tau_v_emg,
  ...
  if (AEBActive != 0) {
    if (rtIsNaN(ChassisInfo->speed)) {
      a_soll_TrafficLightActive = rtNaN;
    } else if (ChassisInfo->speed < 0.0) {
      a_soll_TrafficLightActive = -1.0;
    } else {
      a_soll_TrafficLightActive = (ChassisInfo->speed > 0.0);
    }

    b_a_soll_SpeedPlanAvoidPedestri[0] = -4.0 * a_soll_TrafficLightActive;
    b_a_soll_SpeedPlanAvoidPedestri[1] = a_soll;
    a_soll = c_minimum(b_a_soll_SpeedPlanAvoidPedestri);
  ...
  c_a_soll_SpeedPlanAvoidPedestri[0] = a_soll_SpeedPlanAvoidPedestrian;
  c_a_soll_SpeedPlanAvoidPedestri[1] = t_TargetLaneFront2int_idx_2;
  c_a_soll_SpeedPlanAvoidPedestri[2] = c_a_soll_SpeedPlanAvoidOncoming;
  c_a_soll_SpeedPlanAvoidPedestri[3] = a_soll_TrafficLightActive;
  c_a_soll_SpeedPlanAvoidPedestri[4] = 0.0;
  c_a_soll_SpeedPlanAvoidPedestri[5] = 0.0;
  c_a_soll_SpeedPlanAvoidPedestri[6] = a_soll_ACC;
  c_a_soll_SpeedPlanAvoidPedestri[7] = a_soll_veh2goal;
  g_minimum(c_a_soll_SpeedPlanAvoidPedestri, &d_veh2waitingArea, &b_wait);
  if ((BasicsInfo->d_veh2goal <= 60.0) && (b_wait > 4)) {
    stopdistance_array[7] = BasicsInfo->d_veh2goal;
  }

  d_veh2waitingArea = h_minimum(stopdistance_array);
  ...
  if ((GlobVars->TrajPlanLaneChange.durationLaneChange == 0) &&
        ...
        if (((c_a_soll_SpeedPlanAvoidOncoming <= a_soll_ACC) ||
          ...
          if (fabs(a_soll) <= 1.0E-5) {
            ...
            if ((ChassisInfo->speed > 0.0) && (d_veh2waitingArea > 0.0)) {
              t_TargetLaneFront2int_idx_1 = 3.0 * d_veh2waitingArea / 2.0 /
                ChassisInfo->speed;
              t_TargetLaneFront2int_idx_0 = -8.0 * rt_powd_snf
            ...
            t_TargetLaneFront2int_idx_1 = (3.0 * sqrt(fmax(0.0,
              a_soll_TrafficLightActive + 0.66666666666666663 * a_soll *
              d_veh2waitingArea)) - 2.0 * ChassisInfo->speed) / (a_soll +
              2.2204460492503131E-16);
            t_TargetLaneFront2int_idx_0 = -2.0 * (ChassisInfo->speed + a_soll *
          ...
          if (guard1) {
            for (trueCount = 0; trueCount < 80; trueCount++) {
              t_TargetLaneFront2int_idx_2 = 0.05 * ((double)trueCount + 1.0);
              if (t_TargetLaneFront2int_idx_2 <= t_TargetLaneFront2int_idx_1) {
                a_soll_TrafficLightActive = t_TargetLaneFront2int_idx_2 *
                  t_TargetLaneFront2int_idx_2;
                Trajectory->traj_vs[trueCount] = (speed_tmp_tmp + a_soll *
                  t_TargetLaneFront2int_idx_2) + 0.5 *
                  t_TargetLaneFront2int_idx_0 * a_soll_TrafficLightActive;
                Trajectory->traj_s[trueCount] = ((pos_s + speed_tmp_tmp *
                  t_TargetLaneFront2int_idx_2) + 0.5 * a_soll *
                  a_soll_TrafficLightActive) + 0.16666666666666666 *
                  t_TargetLaneFront2int_idx_0 * rt_powd_snf
                  (t_TargetLaneFront2int_idx_2, 3.0);
              } else {
                Trajectory->traj_vs[trueCount] = 0.0;
                Trajectory->traj_s[trueCount] = pos_s + d_veh2waitingArea;
              }

              Trajectory->traj_vl[trueCount] = 0.0;
              Trajectory->traj_omega[trueCount] = 0.0;
              Trajectory->traj_l[trueCount] = pos_l_CurrentLane;
              Trajectory->traj_psi[trueCount] = 90.0;
            }
          }
        }
      }
    ...
    if (b_wait == 0) {
      ...
      if (a_soll < 0.0) {
        ...
        t_TargetLaneFront2int_idx_0 = fmax(BasicsInfo->v_max, ChassisInfo->speed);
      } else {
      ...
      for (trueCount = 0; trueCount < 80; trueCount++) {
        ...
        if (t_TargetLaneFront2int_idx_2 <= t_TargetLaneFront2int_idx_1) {
          ...
          Trajectory->traj_s[trueCount] = (pos_s + speed_tmp_tmp *
            t_TargetLaneFront2int_idx_2) + 0.5 * a_soll *
            (t_TargetLaneFront2int_idx_2 * t_TargetLaneFront2int_idx_2);
        } else {
          Trajectory->traj_vs[trueCount] = t_TargetLaneFront2int_idx_0;
          Trajectory->traj_s[trueCount] = (pos_s + (t_TargetLaneFront2int_idx_0 *
            t_TargetLaneFront2int_idx_0 - speed_tmp_tmp * speed_tmp_tmp) / (2.0 *
            a_soll + 2.2204460492503131E-16)) + (t_TargetLaneFront2int_idx_2 -
            t_TargetLaneFront2int_idx_1) * t_TargetLaneFront2int_idx_0;
        }
      }
    }
  ...
  for (trueCount = 0; trueCount < 31; trueCount++) {
    b_wait += CalibrationVars->UrbanPlanner.logTrigger[trueCount + 1];
  }
