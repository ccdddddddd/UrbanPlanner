...
if wait==1
    if s_c>-999
        ...
        s_max=0.5*(min([speed+a_max*t_b2int v_max])+speed)*t_b2int;
        s_min=0.5*(max([speed+a_min*t_b2int 0])+speed)*t_b2int;
        prereq1=s_min< min(s_max, s_int-l_b-max([speed+a_min*t_b2int 0])*t_re/GapIndex);
        prereq2=(s_a>10)&&(d_veh2stopline<10);
        prereq3=ACC(v_max,v_b,s_b-l_b,speed,0,CalibrationVars) > 0.25 && s_b-l_b>5;
